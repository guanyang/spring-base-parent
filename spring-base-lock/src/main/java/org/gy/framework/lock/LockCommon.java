...
public class LockCommon implements ImportAware {
    ...
    public DistributedLockAspect distributedLockAspect(ApplicationContext applicationContext) {
        ...
        StringRedisTemplate stringRedisTemplate = applicationContext.getBean(redisTemplateName,
    ...
    public void setImportMetadata(AnnotationMetadata importMetadata) {
        this.enableAsync = AnnotationAttributes.fromMap(
            importMetadata.getAnnotationAttributes(EnableLockAspect.class.getName()));
        if (this.enableAsync == null) {
            throw new IllegalArgumentException(
                "@EnableLockAspect is not present on importing class " + importMetadata.getClassName());
        }
    }
